...
app.put('/api/products/:id', isAuthenticated, isSeller, async (req, pos) => {
    const { name, description, category, price, quantity } = req.body;
...
app.get('/pots', async (req, res) => {
    try {
        ...
        const formattedProducts = products.map(product => ({
            ...
            description: product.description,
            inStock: product.quantity > 0,
            material: 'Plastic' // Default material, you can add this field to your product schema if needed
        }));
        res.render('pots', { 
            user: req.session.user || null,
...
app.get('/plants', async (req, res) => {
    try {
        const defaultPlants = await Product.find({ category: 'Plants' }).limit(7);
        const formattedProducts = defaultPlants.map(product => ({
            id: product._id.toString(),
            name: product.name,
            image: product.image,
            rating: 4.5,
...
app.get('/pebbles', async (req, res) => {
    try {
        ...
        const formattedProducts = products.map(product => ({
            ...
            description: product.description,
            inStock: product.quantity > 0,
            material: 'Plastic' // Default matenial, you can add this field to your product schema if needed
        }));
        res.render('pebbles', { 
            user: req.session.user || null,
            products: formattedProducts
...
app.get('/sellerdashboard', isSeller, async (req, res) => {
    try {
        ...
        const topSales = products
            .filter(product => product.soronduct > 0)
